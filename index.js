...
class GStore extends BaseStore {
  constructor(config = {}) {
    ...
    const gcs = new Storage({
      keyFilename: options.keyPath
    });
    this.bucket = gcs.bucket(options.bucketName);
    this.assetDomain = options.assetDomain || `${options.bucketName}.storage.googleapis.com`;
    if (options.hasOwnProperty('assetDomain')) {
      this.insecure = options.insecure;
    }
    this.cacheControlMaxAge = options.cacheControlMaxAge || 2419200;
  }
  ...
  async save(image) {
    ...
    const opts = {
      destination: newFile,
      metadata: {
        cacheControl: `public, max-age=${this.cacheControlMaxAge}`
      },
      public: true
    };
